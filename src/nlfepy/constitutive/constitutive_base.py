...
class ConstitutiveBase(metaclass=ABCMeta):
    ...
    def constitutive_equation(
        ...
        *,
        ...
        -------
        Cmatrix : ndarray
            Elastic modulus matrix, Cij (n_itg x 6 x 6)
        Rvector : ndarray
            R-vector, Ri (n_itg x 6)
        Stress : ndarray
            Stress vector, Ti (n_itg x 6)
        """
        pass
    ...
    def get_ctensor(self, *, Cij: np.ndarray) -> np.ndarray:
        ...
        C99 = np.concatenate([Cij, Cij[:, :, 3:6]], axis=2)
        C99 = np.concatenate([C99, C99[:, 3:6, :]], axis=1)
        C99 = C99[:, [0, 3, 8, 6, 1, 4, 5, 7, 2], :][:, :, [0, 3, 8, 6, 1, 4, 5, 7, 2]]
    ...
    def get_cmatrix(self, *, Cijkl: np.ndarray) -> np.ndarray:
        ...
        ----------
        Cijkl : ndarray
            Elastic modulus tensor Cijkl (bs x 3 x 3 x 3 x 3)

        Returns
        -------
        Cij : ndarray
            Elastic modulus matrix Cij (bs x 6 x 6)
        """

        return Cijkl.reshape(-1, 9, 9)[:, [0, 4, 8, 1, 5, 6, 3, 7, 2], :][
            :, :, [0, 4, 8, 1, 5, 6, 3, 7, 2]
        ][:, :6, :6]
    ...
    def get_thickness(self, itg: np.ndarray) -> np.ndarray:
        """
    ...
    def calc_stress_elastic(
        ...
        *,
        ...
        Ee = np.matmul(bm, u_disp.T.flatten())
        Ce = self.get_elastic_modulus(n_dof, plane_stress_type)
        Stress = np.einsum("ij, bj -> bi", Ce, Ee)
    ...
    def calc_correction_term_plane_stress_CR(
        ...
        -------
        C_corr : ndarray
            Corrected elastic modulus matrix, Cij (bs x 3 x 3)
        R_corr : ndarray
            Corrected R-vector, Ri (bs x 3)
        """
        ...
        if plane_stress_type == 1:
            ...
            C3333 = Cmatrix[:, 2, 2]
            C_corr -= (
                np.einsum("bi, bj -> bij", Cij33, C33kl)
                / C3333[:, np.newaxis, np.newaxis]
            )
            R_corr -= Cij33 / C3333[:, np.newaxis] * Rvector[:, 2, np.newaxis]
        elif plane_stress_type == 2:
            C356 = self._metal.Cmatrix[:, [2, 4, 5], :][:, :, [2, 4, 5]]
            C356_124 = self._metal.Cmatrix[:, [2, 4, 5], :][:, :, [0, 1, 3]]
            C124_356 = self._metal.Cmatrix[:, [0, 1, 3], :][:, :, [2, 4, 5]]
            Ctmp = np.matmul(np.linalg.inv(C356), C356_124)
            C_corr -= np.matmul(Ctmp, C124_356)
            R_corr -= np.einsum("bij, bj -> bi", Ctmp, Rvector[:, [2, 4, 5]])

        return C_corr, R_corr

    def calc_correction_term_plane_stress_L(
        self, Ltensor: np.ndarray, itg: np.ndarray, plane_stress_type: int = 0
    ) -> np.ndarray:
        """
        Calc. correction term for plane stress condition
        ...
        ----------
        Ltensor : ndarray
            Velocity gradient tensor, Lij (bs x 3 x 3)
        itg : ndarray
            Indices of integral point
        plane_stress_type : int
            Type of plane stress for 2D
        ...
        -------
        L_corr : ndarray
            Corrected velocity gradient tensor, Lij (bs x 3 x 3)
        """
        ...
        if plane_stress_type == 1:
            idx_l = np.where(self._val["cmatrix"][itg, 2, 2] > EPS)
            idx_g = itg[idx_l]
            L_corr[idx_l, 2, 2] = (
                -1.0
                / self._val["cmatrix"][idx_g, 2, 2]
                * (
                    self._val["cmatrix"][idx_g, 2, 0] * L_corr[idx_l, 0, 0]
                    + self._val["cmatrix"][idx_g, 2, 1] * L_corr[idx_l, 1, 1]
                    + self._val["cmatrix"][idx_g, 2, 3] * L_corr[idx_l, 0, 1]
                    - self._val["rvector"][idx_g, 2]
                    + self._val["stress"][idx_g, 2]
                )
            )
        elif plane_stress_type == 2:
            C356 = self._val["cmatrix"][itg][:, [2, 4, 5], :][:, :, [2, 4, 5]]
            idx_l = np.where(np.linalg.det(C356) > EPS)
            idx_g = itg[idx_l]
            C356inv = np.linalg.inv(C356)
            C356_124 = self._val["cmatrix"][itg][:, [2, 4, 5], :][:, :, [0, 1, 3]]
            D3 = np.vstack(
                [Ltensor[:, 0, 0], Ltensor[:, 1, 1], 2.0 * Ltensor[:, 0, 1]]
            ).T
            R356 = self._val["rvector"][itg][:, [2, 4, 5]] - np.einsum(
                "bij, bj -> bi", C356_124, D3
            )
            ...
            D356 = np.einsum("bij, bj -> bi", C356inv, R356)
            L_corr[:, 2, 2] = D356[:, 0]
